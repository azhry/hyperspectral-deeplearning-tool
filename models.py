...
def save_model(model, model_name, dataset_name, **kwargs):
    ...
    model_dir = 'C:/xampp/htdocs/DeepHyperX/checkpoints/' + model_name + "/" + dataset_name + "/"
    if not os.path.isdir(model_dir):
        os.makedirs(model_dir, exist_ok=True)
    if isinstance(model, torch.nn.Module):
        filename = str(datetime.datetime.now()).replace(':', '') + "_epoch{epoch}_{metric:.2f}".format(**kwargs)
